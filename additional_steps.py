...
def map_mlog_ids_to_attributes(
        data_io: DataIO,
    ...
    attributes_encoders = {
        attribute_name: data_io.load_encoders(
            os.path.join(
                data_io.encoders_dir, f"LabelEncoder of {attribute_name}.pkl"
            )
        )
        for attribute_name in attributes_names
    }
    map_data = (
        concat_data_clicked[["mlogId"] + list(attributes_names)]
        .astype(str)
        .drop_duplicates(["mlogId"], ignore_index=True)
    )
    map_data["mlogId"] = mlog_ids_encoder.transform(map_data["mlogId"].values)
    for attribute_name in attributes_names:
        map_data[attribute_name] = attributes_encoders[
            attribute_name
        ].transform(map_data[attribute_name].values)
    map_save_path = os.path.join(maps_save_dir, "mlog_ids_to_attributes.csv")
    map_data.to_csv(map_save_path, index=False)


def get_mlog_ids_tensor(
        session_data: Dict[str, List[Tuple[int, pd.DataFrame]]],
        mlog_encoder: LabelEncoder,
        max_num_clicked_cards: int,
) -> torch.Tensor:
    mlog_ids = []
    for user_id, sessions in session_data.items():
        ...
        for session_id, session in sessions:
            ...
            transformed_mlog_ids = mlog_encoder.transform(
                session["mlogId"].values
            )
            one_session_ids_tensor = torch.from_numpy(
                transformed_mlog_ids.astype(int)
            )
            pad_ids_of_user = nn_func.pad(
                one_session_ids_tensor,
                (0, max_num_clicked_cards - one_session_ids_tensor.shape[0]),
                value=0,
                mode="constant",
            )
    ...
    mlog_ids_tensor = pad_sequence(
        mlog_ids, padding_value=0, batch_first=True
    )  # (I, max_T, max_N)
    return mlog_ids_tensor


def get_flow_index_of_session(
        session: pd.DataFrame,
) -> Tuple[List[str], pd.DataFrame]:
    def compute_impression_position_index(
            impression_pos: pd.Series, threshold: int = 6
    ) -> float:
        diff = impression_pos.diff().fillna(0).astype(int)
        is_continuous = (diff <= threshold).astype(int)
        group_starts = (is_continuous == 0).cumsum()
        groups = [
            impression_pos[group_starts == i].tolist()
            for i in group_starts.unique()
        ]
        length_duration = [
            (len(group), group[-1] - group[0]) for group in groups
        ]
        flow_indicators = [
            length * duration for length, duration in length_duration
        ]
    ...
    for _, sessions in iteration_bar:
        ...
        for _, session in sessions:
            ...
            flow_indices_of_user.append(
                torch.from_numpy(
                    np.unique(
                        session[flow_indices_name].values.astype(np.float32),
                        axis=0,
                    )
                )
            )
        flow_indices_of_user_tensor = torch.stack(flow_indices_of_user).squeeze(
            1
        )
        flow_indices.append(flow_indices_of_user_tensor)
    flow_indices_tensor = pad_sequence(
        flow_indices, padding_value=0, batch_first=True
    )  # (I, max_T, B)
    flow_indices_lookup = {name: i for i, name in enumerate(flow_indices_name)}
    return flow_indices_tensor, flow_indices_lookup


def get_session_interval(
        session_data: Dict[str, List[Tuple[int, pd.DataFrame]]]
) -> Dict[str, List[float]]:
    session_intervals = {}
    for user, sessions in session_data.items():
        session_intervals_of_user = []
        for i in range(1, len(sessions)):
            interval = (
                    sessions[i][1]["timestamp"].iloc[0]
                    - sessions[i - 1][1]["timestamp"].iloc[-1]
            )
            interval = interval.total_seconds() / 60
            session_intervals_of_user.append(interval)
        session_intervals[user] = session_intervals_of_user
    return session_intervals


def extract_mlog_ids_data(
        processed_data_parent_dir_path: str,
        session_data_file_path: str,
    ...
    mlog_ids_tensor = get_mlog_ids_tensor(
        session_data, mlog_ids_encoder, max_num_clicked_cards
    )
    data_io.save_tensor("mlogId", mlog_ids_tensor)
    map_mlog_ids_to_attributes(
        data_io, concat_data, processed_data_parent_dir_path
    )


def extract_flow_indices_data(
        processed_data_parent_dir_path: str, session_data_file_path: str
) -> None:
    encoders_dir = os.path.join(processed_data_parent_dir_path, "encoders")
    tensor_dir = os.path.join(processed_data_parent_dir_path, "tensors")
    data_io = DataIO(
    ...
    session_data = data_io.load_session_data()
    flow_indices_tensor, flow_indices_lookup = get_flow_index_tensor(
        session_data
    )
...
def extract_session_interval_data(
        processed_data_parent_dir_path: str, session_data_file_path: str
) -> None:
    encoders_dir = os.path.join(processed_data_parent_dir_path, "encoders")
    tensor_dir = os.path.join(processed_data_parent_dir_path, "tensors")
    ...
    session_data = data_io.load_session_data()
    session_interval_min = get_session_interval(session_data)
    data_io.save_tensor("sessionIntervals", session_interval_min)


if __name__ == "__main__":
    extract_session_interval_data(
        (
            "min clicked cards num in session 3 min clicked session num 5"
